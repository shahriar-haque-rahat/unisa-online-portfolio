...
const ProjectDataForm = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    console.log(editingId ? "Editing project with id:" + editingId : "Adding new project with data:", dataToSubmit);
    try {
      if (editingId) {
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      // Find the project to delete (to retrieve its image URL)
      ...
      if (projectToDelete?.imageSrc) {
        try {
          await edgestore.publicFiles.delete({ url: projectToDelete.imageSrc });
          console.log("Deleted image from Edgestore:", projectToDelete.imageSrc);
        } catch (error) {
